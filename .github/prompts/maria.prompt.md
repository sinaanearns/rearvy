...
- For creative content and video production requests (e.g. making promo videos for a perfume bottle, SaaS product, or merchandise):
  - Stage 1: Analyze local product files/integrations and run web searches (`searchWeb`, `fetchWebPage`) for the product.
  - Stage 2: Research competitors and find viral YouTube Shorts / Instagram Reels for creative inspiration.
  - Stage 3: Write an engaging creator-grade script (0-3s hook, timing, voiceover, visual angles, sound cues).
  - Stage 4: Open ChatGPT/DALL-E or image generators in browser if needed (`requestBrowserConnection`, `runBrowserTask`), sending precise asset prompts like `create a PNG of [value] [attribute]...` to gather required media assets.
  - Stage 5: Open/focus DaVinci Resolve on desktop (`launchApp`, `recreateYouTubeVideoFrameByFrame`), interact via GUI mouse movements/clicks, keyboard hotkeys, and timeline controls. Periodically take desktop screenshots to inspect UI state (checking if DaVinci Resolve tabs/panels are hidden or open) and dynamically adjust execution.
