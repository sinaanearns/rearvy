...
const { getDesktopAutomationCapabilities } = require("./lib/desktop-automation-backends.cjs");
...
function setupAutomationIPC(ipcMain, { isTrustedSender } = {}) {
  log.debug("Setting up IPC handlers");

  const handle = (channel, handler) => {
    ipcMain.handle(channel, async (event, ...args) => {
      if (typeof isTrustedSender === "function" && !isTrustedSender(event)) {
        throw new Error("Desktop automation request rejected from an untrusted renderer.");
      }
      return handler(...args);
    });
  };

  handle("desktop:automation:start-workflow", async (workflow) => {
    try {
  ...
  handle("desktop:automation:approve-workflow", async (workflowId) => {
    try {
      const executor = getExecutor();
      if (!executor) {
        return { success: false, error: "Executor not initialized" };
  ...
  handle("desktop:automation:reject-workflow", async (workflowId, reason) => {
    try {
      const executor = getExecutor();
      if (!executor) {
        return { success: false, error: "Executor not initialized" };
  ...
  handle("desktop:automation:get-state", async () => {
    const executor = getExecutor();
    return executor ? executor.getState() : null;
  });

  handle("desktop:automation:pause", async () => {
    const executor = getExecutor();
    if (executor) {
      executor.pause();
    }
  ...
  handle("desktop:automation:resume", async () => {
    const executor = getExecutor();
    if (executor) {
  ...
  handle("desktop:automation:stop", async () => {
    const executor = getExecutor();
    if (executor) {
      executor.stop();
    }
  ...
  handle("desktop:automation:get-history", async (workflowId) => {
    const executor = getExecutor();
    return executor ? executor.getHistory(workflowId) : [];
  });

  handle("desktop:automation:test", async () => {
    try {
      const executor = getExecutor();
      if (!executor) {
        return { success: false, error: "Executor not initialized" };
  ...
  handle("desktop:automation:check-app-installed", async ({ appPath } = {}) => {
    try {
  ...
  handle("desktop:automation:get-backend-capabilities", async () => {
    try {
      return { ok: true, ...(await getDesktopAutomationCapabilities()) };
    } catch (error) {
