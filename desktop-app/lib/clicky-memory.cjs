...
const MEMORY_FILE_NAME = "clicky-memory.json";
...
function extractMemorySaveIntent(command) {
  ...
  if (SENSITIVE_MEMORY_PATTERN.test(text)) {
    return {
      ...
      message: "I will not store passwords, keys, tokens, payment details, or recovery phrases in Clicky memory.",
    };
  }
  ...
  const directMemory = matchFirst(text, [
    {
      ...
      toMemory: (value) =>
        buildMemory({
          ...
          kind: "identity",
          value,
          content: `Clicky's preferred assistant name is ${value}.`,
          importance: 9,
...
function detectMemoryQuery(command) {
  ...
  if (/\b(what'?s|what is|do you know|tell me)\s+your name\b/.test(text) || /\bwhat should i call you\b/.test(text)) {
    return { type: "single", key: "assistant.name", fallback: "I am Clicky. You can give me another name if you want me to remember it." };
  }
...
class ClickyMemoryStore {
  constructor(options = {}) {
    this.filePath = options.filePath || this.resolveFilePath();
  }

  resolveFilePath() {
    if (process.env.REARVY_CLICKY_MEMORY_PATH) {
      return process.env.REARVY_CLICKY_MEMORY_PATH;
    }
  ...
  async readState() {
    try {
      ...
      if (error?.code !== "ENOENT") {
        console.warn("[ClickyMemory] Failed to read memory file:", error?.message || error);
      }
  ...
  async answerMemoryQuery(command) {
    ...
    if (query.type === "list") {
      if (memories.length === 0) {
        return { found: false, reply: "I do not have any Clicky memories saved yet." };
      }
