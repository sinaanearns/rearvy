...
function waitForUrl(url, timeout = 30000, interval = 500, options = {}) {
  const requireRearvy = typeof options === "object" ? Boolean(options?.requireRearvy) : Boolean(options);
  ...
  return new Promise((resolve) => {
    ...
    waitLog.debug(
      `Starting with hostname=${hostname}, port=${port}, path=${requestPath}, timeout=${timeout}ms, interval=${interval}ms, requireRearvy=${requireRearvy}`
    );
    ...
    function tryOnce() {
      ...
      const reqMethod = requireRearvy ? "GET" : "HEAD";
      ...
      const req = httpMod.request(
        {
          ...
          timeout: 3000,
          headers: {
            "user-agent": "Rearvy-Desktop-Runtime-Probe/1.0",
          },
        },
        (res) => {
          ...
          if (requireRearvy) {
            ...
            res.on("end", () => {
              ...
              const isRearvyHeader = res.headers["x-rearvy-web"] === "1";
              const isHtml = contentType.includes("text/html") || body.includes("<html") || body.includes("<!DOCTYPE html");
              const isRawJsonApi = contentType.includes("application/json") || (body.trim().startsWith("{") && !body.includes("<html"));

              const isMatch = res.statusCode < 400 && (isRearvyHeader || (isHtml && !isRawJsonApi));

              if (isMatch) {
                waitLog.debug(`Rearvy server verified on ${hostname}:${port}`);
                resolve(true);
              } else {
                waitLog.warn(
                  `Port ${port} responded (status ${res.statusCode}, content-type: ${contentType}), but is NOT a valid Rearvy web server.`
                );
...
function getConfiguredDevUrl() {
  const startPath = normalizeRoutePath(process.env.REARVY_DESKTOP_START_PATH || "/login");
...
async function startLocalWebsiteRuntime({
  ...
  let cwd;
  let envOverrides = {
    REARVY_TRUSTED_LOCAL_RUNTIME: "1",
  };

  if (!isPackaged) {
    const configuredDevUrl = getConfiguredDevUrl();

    if (isLoopbackDevUrl(configuredDevUrl) && await waitForUrl(configuredDevUrl, 1000, 250, { requireRearvy: true })) {
