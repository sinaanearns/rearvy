...
const { executeSemanticDesktopAction } = require("./desktop-automation-backends.cjs");
...
class WorkflowExecutor {
  ...
  /**
   * Prefer a semantic accessibility action before the legacy UIA/coordinate
   * path. Provider failures are deliberately non-fatal here: Windows UIA is
   * the zero-install fallback and preserves existing workflows on machines
   * where an optional provider is not provisioned.
   */
  async trySemanticDesktopAction(operation, action) {
    try {
      return await executeSemanticDesktopAction(operation, action);
    } catch (error) {
      log.debug(`Semantic desktop action (${operation}) unavailable; using native fallback:`, error?.message || error);
      return null;
    }
  }
  ...
  async startWorkflow(workflowInput) {
    if (this.currentWorkflow && ACTIVE_STATES.has(this.currentWorkflow.state)) {
      // Idempotent: if the same workflow is already registered and waiting for
      // approval, return success so the UI can proceed to the Approve button
      // without treating this as an error.
      if (this.currentWorkflow.id === asString(workflowInput?.id || workflowInput?.workflowId)) {
  ...
  async executeAction(action) {
    switch (action.type) {
      ...
      case "readVisibleText":
        {
          const semantic = await this.trySemanticDesktopAction("list", action);
          if (semantic?.value?.elements) {
            const items = semantic.value.elements
              .map((element) => ({
                text: asString(element?.name),
                controlType: asString(element?.controlType),
                x: Number(element?.x) || 0,
                y: Number(element?.y) || 0,
                width: Number(element?.width) || 0,
      ...
      case "getElementState":
        {
          const semantic = await this.trySemanticDesktopAction("state", action);
          return semantic?.value ? { ...semantic.value, provider: semantic.provider } : await getWindowsElementState(action);
        }

      case "getElementValue":
        {
          const semantic = await this.trySemanticDesktopAction("value", action);
          return semantic?.value ? { ...semantic.value, provider: semantic.provider } : await getWindowsElementValue(action);
        }

      case "invokeElement": {
        const semantic = await this.trySemanticDesktopAction("invoke", action);
        const element = semantic?.value || await invokeWindowsUiElement(action);
        return [
          `Invoked UI element "${element.name || action.text || action.label || action.target}".`,
          `Type: ${element.controlType || "unknown"}.`,
          semantic ? `Provider: ${semantic.provider}.` : "",
      ...
      case "listUiElements":
        {
          const semantic = await this.trySemanticDesktopAction("list", action);
          return semantic?.value?.elements
            ? { ...semantic.value, provider: semantic.provider }
            : await listWindowsUiElements(action);
        }
      ...
      case "waitForElement": {
        const semantic = await this.trySemanticDesktopAction("wait", action);
        const element = semantic?.value || await findWindowsUiElement(action);
        return [
      ...
      case "clickElement": {
        const semantic = await this.trySemanticDesktopAction("click", action);
        if (semantic?.value) {
          const element = semantic.value;
          return [
            `${action.double ? "Double-clicked" : "Clicked"} ${normalizeMouseButton(action.button)} button on UI element "${element.name || action.text}".`,
            `Type: ${element.controlType || "unknown"}.`,
      ...
      case "typeIntoElement": {
        const semantic = await this.trySemanticDesktopAction("type", action);
        if (semantic?.value) {
          const value = asString(action.value ?? action.textToType ?? action.input ?? action.content ?? action.text);
          return [
            `Typed into UI element "${semantic.value.name || action.label || action.target || action.text}".`,
            `Provider: ${semantic.provider}.`,
      ...
      case "setElementValue": {
        const semantic = await this.trySemanticDesktopAction("setValue", action);
        const element = semantic?.value || await setWindowsElementValue(action);
        return [
      ...
      case "selectOption": {
        const semantic = await this.trySemanticDesktopAction("select", action);
        if (semantic?.value) {
          const targetLabel = asString(action.text || action.label || action.name || action.target);
          return `Selected option on "${semantic.value.name || targetLabel}" with ${semantic.provider}.`;
        }
      ...
      case "focusElement": {
        const semantic = await this.trySemanticDesktopAction("focus", action);
        if (semantic?.value) {
          return `Focused element "${semantic.value.name || action.text || action.label}" with ${semantic.provider}.`;
        }
        const element = await findWindowsUiElement(action);
        return `Focused element "${element.name || action.text || action.label}".`;
      }

      case "scrollElement": {
        const semantic = await this.trySemanticDesktopAction("scroll", action);
        if (semantic?.value) {
          return `Scrolled element "${semantic.value.name || action.text || action.label}" with ${semantic.provider}.`;
        }
        return `Scrolled element "${action.text || action.label || "target"}".`;
      }

      case "sendShortcut": {
        const semantic = await this.trySemanticDesktopAction("shortcut", action);
        if (semantic?.value) {
          return `Sent shortcut "${action.keys || action.shortcut}" to "${semantic.value.name || action.text}" with ${semantic.provider}.`;
        }
        const nativeRobot = requireRobot("sendShortcut");
        const keys = asString(action.keys || action.shortcut);
        nativeRobot.typeString(keys);
        return `Sent shortcut "${keys}".`;
      }

      case "setToggleState": {
        const semantic = await this.trySemanticDesktopAction("toggle", action);
        if (semantic?.value) {
          const label = asString(action.text || action.label || action.name || action.target);
          return `Set toggle "${semantic.value.name || label}" with ${semantic.provider}.`;
        }
        ...
        const requestedState = action.state ?? action.checked ?? action.value ?? action.mode ?? "toggle";
        const state = typeof requestedState === "boolean"
          ? requestedState ? "checked" : "unchecked"
          : asString(requestedState, "toggle").toLowerCase();
      ...
      case "importDaVinciTimeline":
        return await this.importDaVinciTimeline(action);

      case "downloadYouTubeVideo":
        return await this.downloadYouTubeVideo(action);
  ...
  /**
   * Imports an FCPXML or EDL timeline file into a running DaVinci Resolve
   * session via Windows UI Automation + keyboard/mouse automation.
   *
   * Steps:
   *   1. Verify the file exists.
   *   2. Focus or launch DaVinci Resolve.
   *   3. Trigger File > Import > Timeline via Alt+F, I, T keyboard sequence.
   *   4. Paste the file path into the system Open dialog and confirm.
   *   5. Wait for import to settle; take a verification screenshot.
   *   6. Optionally trigger Quick Export (Ctrl+Shift+E) and await render.
   *
   * @param {object} action
   * @param {string}  action.timelinePath        Absolute path to .fcpxml or .edl
   * @param {string}  [action.timelineName]      Display name for logs
   * @param {boolean} [action.renderAfterImport] Trigger Quick Export after import
   * @param {string}  [action.renderOutputPath]  Where to save the rendered video
   * @param {number}  [action.importSettleMs]    Extra settle wait (default 2500)
   * @param {number}  [action.renderWaitMs]      Max render wait ms (default 120000)
   */
  async importDaVinciTimeline(action) {
    const timelinePath = asString(
      action.timelinePath || action.path || action.filePath || action.file
    );
    if (!timelinePath) {
      throw new Error(
        'importDaVinciTimeline requires timelinePath (absolute path to .fcpxml or .edl).'
      );
    }

    const timelineName = asString(
      action.timelineName || action.name,
      path.basename(timelinePath, path.extname(timelinePath))
    );
    const renderAfterImport = action.renderAfterImport === true;
    const renderOutputPath = asString(
      action.renderOutputPath || action.outputPath,
      path.join(
        process.env.USERPROFILE || 'C:\\Users\\Public',
        'Desktop',
        `${timelineName}_render.mp4`
      )
    );
    const importSettleMs = Math.max(
      1000,
      Math.min(15000, Number(action.importSettleMs) || 2500)
    );
    const renderWaitMs = Math.max(
      10000,
      Math.min(600000, Number(action.renderWaitMs) || 120000)
    );

    log.info(
      `importDaVinciTimeline: importing "${timelinePath}" ` +
      `(renderAfterImport=${renderAfterImport})`
    );

    // ── 1. Verify the timeline file exists ───────────────────────────────────
    try {
      await fs.access(timelinePath);
    } catch {
      throw new Error(
        `importDaVinciTimeline: timeline file not found at "${timelinePath}".`
      );
    }

    // ── 2. Focus or raise DaVinci Resolve window ─────────────────────────────
    const RESOLVE_TITLE = 'DaVinci Resolve';
    let resolveReady = false;

    const tryFocus = async () => {
      try {
        await focusWindowsWindow({ windowTitle: RESOLVE_TITLE });
        resolveReady = true;
      } catch {
        // not yet visible
      }
    ...
    if (!resolveReady) {
      log.info('importDaVinciTimeline: Resolve not focused — attempting launch...');
      try {
        await this.launchApp({ type: 'launchApp', appPath: 'Resolve.exe', wait: true });
      } catch (launchErr) {
        log.warn(`importDaVinciTimeline: launch attempt: ${launchErr.message}`);
      }

      for (let attempt = 0; attempt < 12 && !resolveReady; attempt++) {
        await sleep(2000);
        await tryFocus();
      }
      ...
      if (!resolveReady) {
        throw new Error(
          'importDaVinciTimeline: DaVinci Resolve window not found after launch. ' +
          'Please open DaVinci Resolve manually and try again.'
        );
      }
    }

    log.info('importDaVinciTimeline: DaVinci Resolve focused.');
    await sleep(800);
    await this.ensureRunnable();

    // ── 3. Open File → Import → Timeline via keyboard sequence ───────────────
    // Alt+F opens the File menu in DaVinci Resolve on Windows.
    log.info('importDaVinciTimeline: opening File > Import > Timeline...');

    const pressKey = async (combo) => {
      try {
        await agentDesktop.press(combo);
      } catch (agentErr) {
        log.debug(`agentDesktop.press("${combo}") failed, using robotjs:`, agentErr?.message);
        const parts = combo.split('+');
        const key = parts[parts.length - 1];
        const mods = parts.slice(0, -1).map(mapModifier);
        const nativeRobot = requireRobot('importDaVinciTimeline');
        if (mods.length > 0) {
          nativeRobot.keyTap(key, mods);
        } else {
          nativeRobot.keyTap(key);
        }
      }
    };

    await pressKey('alt+f');   // Open File menu
    await sleep(400);
    await pressKey('i');        // Hover Import submenu
    await sleep(400);
    await pressKey('t');        // Click Timeline...
    await sleep(1200);          // Wait for system file-open dialog
    await this.ensureRunnable();

    // ── 4. Paste the file path into the Open dialog ──────────────────────────
    log.info(`importDaVinciTimeline: pasting path "${timelinePath}"...`);
    try {
      await agentDesktop.clipboardSet(timelinePath);
    } catch {
      try { clipboard.writeText(timelinePath); } catch { /* ignore */ }
    }
    await sleep(150);
    ...
    log.info('importDaVinciTimeline: confirmed Open dialog.');

    // ── 5. Wait for import and take a verification screenshot ─────────────────
    log.info(`importDaVinciTimeline: settling for ${importSettleMs}ms...`);
    await sleep(importSettleMs);
    await this.ensureRunnable();

    let verifyScreenshot = 'Screenshot not captured.';
    try {
      verifyScreenshot = await this.captureScreenshot();
    } catch (screenshotErr) {
      log.warn('importDaVinciTimeline: post-import screenshot failed:', screenshotErr?.message);
    }

    let renderResult = 'Render not requested.';

    // ── 6 (optional). Quick Export to produce the video file ──────────────────
    if (renderAfterImport) {
      log.info(
        `importDaVinciTimeline: triggering Quick Export → "${renderOutputPath}"...`
      );
      await this.ensureRunnable();
      await sleep(500);

      // Ctrl+Shift+E opens the Quick Export dialog
      await pressKey('ctrl+shift+e');
      await sleep(2000);
      await this.ensureRunnable();

      // Try clicking the Export/Start Render button by accessible name
      let exportClicked = false;
      for (const label of ['Export', 'Start Render', 'Render', 'OK']) {
        try {
          await invokeWindowsUiElement({
            text: label,
            controlType: 'Button',
            timeoutMs: 3000,
          });
          exportClicked = true;
          log.info(`importDaVinciTimeline: clicked "${label}" in Quick Export dialog.`);
          break;
        } catch {
          // try next label
        }
      ...
      if (!exportClicked) {
        // Fallback: Enter confirms the default action
        await pressKey('enter');
        log.warn('importDaVinciTimeline: Export button not found by label; pressed Enter.');
      }

      // Poll for completion (file existence + UI text scan)
      const renderDeadline = Date.now() + renderWaitMs;
      let renderComplete = false;
      log.info(`importDaVinciTimeline: waiting up to ${renderWaitMs / 1000}s for render...`);

      while (Date.now() < renderDeadline && !renderComplete) {
        await this.ensureRunnable();
        await sleep(3000);

        // Signal 1: output file exists
        try {
          await fs.access(renderOutputPath);
          renderComplete = true;
          log.info(`importDaVinciTimeline: render output found at "${renderOutputPath}".`);
          break;
        } catch {
          // not yet written
        }

        // Signal 2: completion phrase visible in UI
        try {
          const { text } = await readWindowsVisibleText({ maxTextItems: 60 });
          if (/(?:render complete|export complete|job complete|finished|done)/i.test(text || '')) {
            renderComplete = true;
            log.info('importDaVinciTimeline: render completion detected via UI text.');
          }
        } catch {
          // ignore
        }
      ...
      renderResult = renderComplete
        ? `Render complete. Output: "${renderOutputPath}".`
        : `Render started but did not confirm completion within ${renderWaitMs / 1000}s. ` +
          `Check DaVinci Resolve for status.`;
    }

    return [
      `DaVinci Resolve timeline import triggered for "${timelineName}".`,
      `File: "${timelinePath}".`,
      renderResult,
    ].join(' ');
  }
  ...
  /**
   * Downloads a YouTube (or any yt-dlp-supported) video to disk.
   * Requires yt-dlp to be installed and on PATH.
   *
   * @param {object} action
   * @param {string}  action.url          Video URL
   * @param {string}  [action.outputPath] Output path template (default: Desktop)
   * @param {string}  [action.format]     yt-dlp format selector
   */
  async downloadYouTubeVideo(action) {
    const url = asString(action.url || action.videoUrl || action.link);
    if (!url) {
      throw new Error('downloadYouTubeVideo requires url.');
    }
    ...
    try {
      ...
      throw new Error(`downloadYouTubeVideo: invalid URL "${url}": ${urlErr.message}`);
    }
    ...
    const outputTemplate = asString(
      action.outputPath || action.output || action.destination,
      path.join(desktopDir, '%(title)s.%(ext)s')
    );
    ...
    log.info(`downloadYouTubeVideo: url="${url}" output="${outputTemplate}"`);
      ...
      try {
        const result = await readChildProcessOutput(
          ...
          [
            ...
            '-NonInteractive',
            '-Command',
            `(Get-Command ${JSON.stringify(candidate)} -ErrorAction SilentlyContinue)?.Source`,
          ],
          { windowsHide: true }
        );
    ...
    if (!ytdlpExe) {
      throw new Error(
        'downloadYouTubeVideo: yt-dlp is not installed or not on PATH. ' +
        'Install it with: winget install yt-dlp.yt-dlp  or  pip install yt-dlp'
      );
    }

    // Ensure output directory exists
    const outputDir = outputTemplate.includes('%(')
      ? desktopDir
    ...
    log.info(`downloadYouTubeVideo: running ${ytdlpExe} ${args.join(' ')}`);

    const result = await readChildProcessOutput(ytdlpExe, args, {
      env: { ...process.env },
    });

    // Extract resolved output path from yt-dlp stdout
    const destMatch = result.stdout.match(
      /\[(?:download|Merger)\]\s+(?:Destination|Merging):\s+(.+)/
    );
    const resolvedPath = destMatch?.[1]?.trim() || outputTemplate;

    return [
      `Downloaded video from "${url}".`,
      `Output: "${resolvedPath}".`,
      result.stdout ? `Log: ${result.stdout.slice(-300)}` : '',
    ]
      .filter(Boolean)
      .join(' ');
  }
