...
async function captureInitialDeviceProfile() {
  ...
  ipcMain.handle("desktop:system:capture-screen", async (event, opts = {}) => {
    const targetApp = typeof opts === "string" ? opts : opts?.targetApp || opts?.app || null;
    ...
    if (targetApp && Array.isArray(sources)) {
      ...
      if (matchingWindow && matchingWindow.thumbnail && typeof matchingWindow.thumbnail.toDataURL === "function") {
        const url = matchingWindow.thumbnail.toDataURL();
        if (url && !matchingWindow.thumbnail.isEmpty?.()) {
          return url;
        }
      }
    }
  ...
  // Approval-gated desktop workflows are available only to Rearvy's trusted
  // local/packaged origins; an embedded or remote renderer cannot invoke OS control.
