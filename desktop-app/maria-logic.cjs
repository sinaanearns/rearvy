...
class MariaBrain {
  ...
  async captureDesktopScreens(options = {}) {
    const targetApp = typeof options === "string" ? options : options?.targetApp || options?.app || null;
    ...
    const windowScreenshots = [];
    if (targetApp && Array.isArray(sources)) {
      const cleanTarget = String(targetApp).toLowerCase().trim();
      const matchingWindows = sources.filter(
        (s) => s.id?.startsWith("window:") && s.name && s.name.toLowerCase().includes(cleanTarget)
      );
      for (const source of matchingWindows) {
        const image = source?.thumbnail && typeof source.thumbnail.toDataURL === "function"
          ? source.thumbnail.toDataURL()
          : "";
        if (!image || source.thumbnail?.isEmpty?.()) continue;
        const thumbnailSize = typeof source.thumbnail?.getSize === "function" ? source.thumbnail.getSize() : null;
        windowScreenshots.push({
          image,
          label: `Background Target Window: ${source.name}`,
          isCursorScreen: true,
          isTargetWindow: true,
          bounds: null,
    ...
    const screenSources = (sources || []).filter((s) => s.id?.startsWith("screen:") || !s.id?.startsWith("window:"));
    const screenshots = screenSources
      .map((source, index) => {
        const image = source?.thumbnail && typeof source.thumbnail.toDataURL === "function"
          ? source.thumbnail.toDataURL()
          : "";
        ...
        return {
          image,
          label: screenSources.length === 1
            ? "User screen (cursor is here)"
            : isCursorScreen
              ? `Screen ${index + 1} of ${screenSources.length} - cursor is here`
              : `Screen ${index + 1} of ${screenSources.length}`,
          isCursorScreen,
          bounds: bounds
            ? {
                x: Number(bounds.x) || 0,
    ...
    const combined = [...windowScreenshots, ...screenshots.sort((a, b) => Number(b.isCursorScreen) - Number(a.isCursorScreen))];
    return combined;
  }

  async captureDesktopScreen(options = {}) {
    const screenshots = await this.captureDesktopScreens(options);
    return screenshots[0]?.image || null;
  }

  async perceiveScreenContext(options = {}) {
    const preferDesktop = Boolean(options.preferDesktop || options.targetApp || options.app);

    try {
      if (preferDesktop) {
        try {
          const desktopScreenshots = await this.captureDesktopScreens(options);
