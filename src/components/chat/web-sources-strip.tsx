...
export function WebSourcesStrip({
  ...
  return (
    <details className="group w-full max-w-2xl rounded-2xl border border-border/60 bg-card/60 shadow-sm backdrop-blur-sm">
        ...
        <div className="flex min-w-0 items-center gap-2">
          <div className="hidden max-w-[20rem] items-center gap-1 overflow-hidden sm:flex">
            {sources.slice(0, 3).map((source) => (
              <span
                key={source.url}
                className="truncate rounded-full border border-border/60 bg-background/70 px-2 py-0.5 text-[11px] text-muted-foreground"
              >
                {source.source}
              </span>
            ))}
        ...
        <div className="space-y-2">
          {sources.slice(0, 5).map((source) => (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="block rounded-xl border border-border/60 bg-background/50 p-3 transition-colors hover:bg-muted/30"
            >
