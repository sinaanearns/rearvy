...
import { DEFAULT_PLAN, FREE_PLAN_CREDITS, FREE_PLAN_CREDITS_LABEL, REARVY_PLANS, getPlanDefinition, type SubscriptionPlan } from "@/lib/plans";
...
export default function ProfilePage() {
  ...
  const planLabel = planDef.name;
  const isPaidPlan = plan === "pro" || plan === "business";
  const credits = typeof profile.credits === "number" ? profile.credits : (isPaidPlan ? Infinity : FREE_PLAN_CREDITS);
  const creditsLabel = isPaidPlan
    ? (typeof profile.credits === "number" && profile.credits > 1000 ? `${profile.credits.toLocaleString("en-US")} credits` : "Unlimited credits")
