...
import { DEFAULT_PLAN, FREE_PLAN_CREDITS_LABEL, REARVY_PLANS, getPlanDefinition, type SubscriptionPlan } from "@/lib/plans";
import {
...
export default function SettingsPage() {
  ...
  const activePlanDef = getPlanDefinition(profile.plan);
  const activePlanLabel = activePlanDef.name;
  ...
  const settingsHighlights = [
    ...
    },
    {
      ...
      helper: isPaidPlan ? "Unlimited credits active" : `${FREE_PLAN_CREDITS_LABEL} on free access`,
      icon: Coins,
    },
    {
      label: "Security",
  ...
  async function handleSyncSubscription() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Could not sync subscription status.");
      }

      setProfile((prev) => ({ ...prev, plan: "business" }));
      toast.success("Subscription status synced. Business Access is active!");
    } catch (error) {
      log.error("Sync subscription failed:", error);
      toast.error(getErrorMessage(error, "Failed to sync subscription status."));
    } finally {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-8 pb-10 pt-4">
      <Tabs defaultValue="profile" className="w-full space-y-6">
        <TabsList className="flex h-auto w-full gap-2 rounded-[8px] border border-border/70 bg-muted/30 p-2 shadow-sm shadow-slate-950/[0.03] [&>*]:flex-1">
          <TabsTrigger
            value="profile"
              ...
              <CardContent className="space-y-6 p-5 sm:p-6">
                ...
                <div className="grid gap-6 md:grid-cols-2">
                  ...
                  <div className="space-y-2 md:col-span-2">
                    ...
                    />
                    <p className="text-[10px] text-muted-foreground italic">
                      Add one link per line (website, GitHub, portfolio, etc.).
                    </p>
                  </div>
              ...
              <CardHeader className={settingsProfileHeaderClass}>
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <CardDescription>
                      {isPaidPlan
                        ? `Your account is active on ${activePlanLabel} with unlimited credits.`
              ...
              <CardContent className="space-y-4 p-5 sm:p-6">
                <div className="relative overflow-hidden rounded-[8px] border border-emerald-200/55 bg-background/82 p-5 shadow-sm shadow-slate-950/[0.03] dark:border-emerald-200/22 dark:bg-white/[0.04]">
                  ...
                  <div className="mb-4 flex items-start justify-between gap-4">
                    <div className="relative">
                      <div className="flex items-center gap-2">
                        <span className="text-lg font-semibold">{activePlanDef.name}</span>
                        {isPaidPlan && (
                          <span className="rounded-[8px] bg-emerald-500/15 border border-emerald-500/30 px-2.5 py-0.5 text-xs font-semibold text-emerald-600 dark:text-emerald-300">
                            Active Plan
                          </span>
                        )}
                      </div>
                      <p className="mt-1 text-sm text-muted-foreground">
                        {activePlanDef.description}
                      </p>
                    </div>
                    <div className="relative text-right">
                      <div className="text-2xl font-bold">{activePlanDef.price}</div>
                      <div className="text-xs text-muted-foreground">{activePlanDef.period}</div>
                    </div>
                  </div>

                  <div className="relative space-y-2">
                    {activePlanDef.features.map((feature) => (
                      <div
                        key={feature}
                ...
                {!isPaidPlan ? (
                  <div className="rounded-[8px] border border-cyan-500/20 bg-cyan-500/5 p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                    <div>
                      <p className="text-sm font-semibold text-foreground">Upgrade to Business Access</p>
                      <p className="text-xs text-muted-foreground">Already paid or upgraded? Click Sync Recent Upgrade to activate immediately.</p>
                    </div>
                    <div className="flex flex-wrap items-center gap-2 shrink-0">
                      ...
                      >
                        {syncingSubscription && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                        Sync Recent Upgrade
                      </Button>
                ...
                <div className={cn(settingsProfileTileClass, "p-5")}>
                  ...
                  <div className="mt-2 flex flex-col gap-2 sm:flex-row">
                    <Input
                      ...
                      placeholder="Enter code (e.g. BUSINESS)"
                      className="bg-background-muted shadow-none"
                    />
                    <Button
                      type="button"
