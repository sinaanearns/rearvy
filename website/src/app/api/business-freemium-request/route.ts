...
export async function POST(request: NextRequest) {
  try {
    ...
    const about = readString(body?.about);
    const hopes = readString(body?.hopes);
    const thingsToImplement = readString(body?.thingsToImplement);
    const featuresWanted = readString(body?.featuresWanted);
    ...
    // Always attempt to save to Firestore first so data is never lost
    try {
      await adminDb.collection(COLLECTIONS.BUSINESS_REGISTRATIONS).add({
        businessName,
        gmail,
        plannedUse,
        about,
      ...
      log.error("Failed to persist registration to Firestore:", dbError);
    }
    ...
    if (
      ...
      try {
        ...
        log.warn("Resend email failed but registration was persisted:", emailErr);
      }
    }
    ...
    log.error("Error handling business freemium request:", error);
    return NextResponse.json(
      { error: "Failed to submit business freemium request." },
      { status: 500 }
    );
  }
}
