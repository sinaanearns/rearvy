...
import { detectChatGptImageIntent } from "@/lib/ai/chatgpt-image-intent";
...
function getBrowserExecutionProgress(status: string | null, summary: string | null) {
  ...
  if (status === "running" || status === "initializing" || status === "processing_command") {
    return "Rearvy started the browser session and is logging each step in the execution log.";
  }
...
function buildBrowserExecutionSummary(params: {
  ...
  const isSignupFlow =
    ...
    (/\b(sign\s*up|signup|register|account creation|create an? account)\b/i.test(
      ...
      !/\b(generate|create|draw|paint|make)\s+(?:an?\s+)?(?:image|picture|photo|illustration)\b/i.test(
        browserTaskInstruction
      ));
  ...
  const intro = isSignupFlow
    ? signupEmail
      ...
      : `Working on ${targetLabel} signup. If account email is unknown, I will open Google account chooser and ask which account to use.`
    : `Working on the requested task on ${targetLabel}.`;
...
export async function POST(req: NextRequest) {
  ...
  if (shouldForceMapGeneration && mapGenerationIntent && resolvedChatId) {
    ...
    const toolCallId = `${toolName}-${crypto.randomUUID()}`;
    const nowIso = new Date().toISOString();
    let toolInput: GenerateMapInput | undefined;
    let mapGenerationSuccessMessage: string | undefined;

    if (mapGenerationIntent.kind === "jp-morgan") {
      toolInput = mapGenerationIntent.input;
    } else if (mapGenerationIntent.kind === "geocode-map") {
      const geocodeExecute = tools?.geocode?.execute as
        | ((input: { query: string }, context?: { toolCallId?: string; messages?: Array<Record<string, unknown>> }) => Promise<unknown>)
        | undefined;
      if (!geocodeExecute) {
        return createDeterministicTextChatResponse({
          chatId: resolvedChatId,
          assistantText:
            "I couldn't look up the location because geocoding is not enabled for this chat.",
          metadata: {
            model: selectedProviderModel,
            defaultModel: modelOption.providerModel,
            modelTier: aiModel,
      ...
      let geocodeToolOutput: unknown;
      try {
        geocodeToolOutput = await geocodeExecute(
          { query: mapGenerationIntent.query },
          {
            toolCallId: `${toolName}-geocode-${crypto.randomUUID()}`,
            messages: outboundModelMessages,
          }
        );
      } catch (error) {
        geocodeToolOutput = {
          ...
          message: getReadableErrorMessage(error, "Failed to look up the location."),
        };
      }
      const geocodeResult = geocodeToolOutput as { latitude?: number; longitude?: number; display_name?: string } | undefined;
      if (!geocodeResult || typeof geocodeResult.latitude !== "number" || typeof geocodeResult.longitude !== "number") {
        const failureMessage =
          typeof (geocodeToolOutput as { message?: string })?.message === "string"
            ? (geocodeToolOutput as { message?: string }).message
            : "Failed to look up the location.";
        return createDeterministicTextChatResponse({
          chatId: resolvedChatId,
          assistantText: `I couldn't find the location for "${mapGenerationIntent.query}". ${failureMessage}`,
          metadata: {
            model: selectedProviderModel,
            defaultModel: modelOption.providerModel,
            modelTier: aiModel,
      ...
      toolInput = {
        title: `Map for ${mapGenerationIntent.query}`,
        summary: `Location: ${geocodeResult.display_name}`,
        focus: mapGenerationIntent.query,
        markers: [
          {
            latitude: geocodeResult.latitude,
            longitude: geocodeResult.longitude,
            label: mapGenerationIntent.query,
            description: geocodeResult.display_name,
          },
        ],
        routes: [],
      };
      mapGenerationSuccessMessage = `Here is the map for "${mapGenerationIntent.query}"`;
    }
    ...
    if (!generateMapExecute || !toolInput) {
      return createDeterministicTextChatResponse({
        chatId: resolvedChatId,
        assistantText:
    ...
    const assistantText = toolFailed
      ...
      : (mapGenerationSuccessMessage ?? mapGenerationIntent.assistantText);
    const assistantContent: Array<Record<string, unknown>> = [
      {
        type: "tool-call",
        toolCallId,
...
- After the tool returns, say that the screenshot workflow will run automatically in the Desktop Workspace. Do not claim the screenshot has already been captured until workflow evidence is available. Do not generate text-based approval prompts or prompt the user for unrequested follow-up workflows.
