...
import { getVoiceContext, getVoiceProfile, updateVoiceProfile } from "@/lib/clicky/voice-store";
import type { ClickyVoiceProfile } from "@/lib/clicky/voice-core";
...
function sanitizeProfilePatch(body: Record<string, unknown>): Partial<ClickyVoiceProfile> {
  const patch: Partial<ClickyVoiceProfile> = {};
  ...
  if (body.styleDefaults && typeof body.styleDefaults === "object") {
    patch.styleDefaults = body.styleDefaults as ClickyVoiceProfile["styleDefaults"];
  }
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.error("Failed to load Clicky voice profile:", error);
    return NextResponse.json({ error: "Failed to load Clicky voice profile." }, { status: 500 });
  }
}
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    console.error("Failed to update Clicky voice profile:", error);
    return NextResponse.json({ error: "Failed to update Clicky voice profile." }, { status: 500 });
  }
}
