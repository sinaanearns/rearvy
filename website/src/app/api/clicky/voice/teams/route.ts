...
import { createVoiceTeam, listVoiceMemberships, listVoiceTeams } from "@/lib/clicky/voice-store";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.error("Failed to list Clicky voice teams:", error);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const name = readString(body?.name, "Clicky Team", 160);
    ...
    console.error("Failed to create Clicky voice team:", error);
