...
export async function POST(
  ...
  if (!targetUser) {
    const inviteRef = adminDb.collection(COLLECTIONS.MARIA_VOICE_TEAM_INVITES).doc();
    const invite = {
      team_id: id,
      email,
      role,
  ...
  const existing = await adminDb
    ...
    .limit(1)
    .get();
  const memberRef = existing.docs[0]?.ref || adminDb.collection(COLLECTIONS.MARIA_VOICE_TEAM_MEMBERS).doc();
  const member = {
    team_id: id,
