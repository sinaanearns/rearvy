...
import {
  readBrowserConnectionPreference,
  writeBrowserConnectionPreference,
...
const CHROME_EXTENSION_URL = "https://chromewebstore.google.com/search/Rearvy";
...
function normalizeMethods(value: unknown): BrowserConnectionMethod[] {
  ...
  const allowed = new Set(["cdp-direct", "extension-relay", "managed-runner", "firecrawl"]);
  return methods
...
export function BrowserConnectionCard({
  ...
  const connectRearvy = useCallback(async () => {
    writeBrowserConnectionPreference("extension-relay");
    if (typeof window !== "undefined") {
  ...
  useEffect(() => {
    ...
    const savedPreference = readBrowserConnectionPreference();

    // 1) Live connection is active (Extension / CDP Direct)
    if (methodConnected(status, method)) {
      writeBrowserConnectionPreference(method);
      autoContinueSubmittedRef.current = true;
      void submit("connected");
      return;
    }

    // 2) User has previously selected a connection choice (e.g. "managed-runner" via "Open Browser Directly")
    // Auto-submit immediately using their saved choice so they are NEVER asked again!
    if (
      savedPreference &&
      toolCallId &&
      onToolOutput &&
      !hasSubmittedConnectionToolCall(toolCallId)
    ) {
      autoContinueSubmittedRef.current = true;
      markSubmittedConnectionToolCall(toolCallId);
      setIsSubmitting(true);
      ...
      void (async () => {
        try {
          await onToolOutput({
            ...
            output: {
              status: "connected",
              method: savedPreference,
              message:
                savedPreference === "firecrawl"
                  ? "Proceeding with Firecrawl Cloud Browser based on user preference."
                  : savedPreference === "managed-runner"
                    ? "Proceeding with Firecrawl Cloud Browser."
                    : `Connected using saved browser preference (${savedPreference}).`,
  ...
  }, [canRespond, isSubmitting, method, onToolOutput, relayInfo, status, submit, toolCallId]);

  const openBrowserDirectly = useCallback(async () => {
    writeBrowserConnectionPreference("managed-runner");
    const bridge = getBrowserBridge();
