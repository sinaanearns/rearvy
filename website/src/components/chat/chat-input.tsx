...
import type { ChatPermissionMode, DesktopWorkspaceScope } from "@/lib/chat/permissions";
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex min-w-0 items-end gap-1.5 rounded-[8px] border border-border/70 bg-card/85 p-1.5 shadow-sm shadow-slate-950/[0.04] backdrop-blur-xl sm:gap-2 sm:p-2">
          ...
          {model && onModelChange && (
            <ModelSelectorMenu
              ...
              disabled={isLoading}
              className="h-10 sm:h-[44px] shrink-0"
            />
          )}
        ...
        <div className="mx-2 flex flex-wrap items-center gap-2 self-start">
          {onPermissionModeChange && (
            <button
              type="button"
              onClick={() =>
                onPermissionModeChange(
                  permissionMode === "full-access" ? "approval" : "full-access"
                )
              }
              className={cn(
                "flex items-center gap-1.5 rounded-[8px] border px-2.5 py-1 text-xs font-medium transition-all shadow-sm",
              ...
              title={
                permissionMode === "full-access"
                  ? "Full Access System: Executes actions automatically without warnings or approval prompts"
                  : "Approval System: Asks before AI uses terminal or performs security-sensitive actions"
              }
            >
              <span
                className={cn(
              ...
              />
              <span>
                {permissionMode === "full-access" ? "Full Access System" : "Approval System"}
              </span>
            </button>
          )}
          ...
          {onPickWorkspaceFolder && hasWorkspaceScope && (
            <button
              ...
              className="flex min-w-0 items-center gap-2 rounded-[8px] px-2 py-1.5 text-xs font-medium text-muted-foreground transition-colors hover:bg-muted/70 hover:text-foreground"
