...
import { buildAssistantTimelineEntries } from "@/lib/chat/assistant-timeline";
...
function isExecutionProgressNarration(text: string) {
  const normalized = text.trim().replace(/\s+/g, " ").toLowerCase();

  return (
    /\bnow i (?:have|need|will)\b/.test(normalized) ||
    /\blet me(?: now)?\b/.test(normalized) ||
    /\b(?:start|begin) (?:building|creating|generating)\b/.test(normalized) ||
    /\bcomprehensive understanding\b/.test(normalized)
  );
}

function getConciseExecutionAnalysis(texts: string[]) {
  const content = texts.join(" ").replace(/\s+/g, " ").toLowerCase();

  if (/\brecreation plan is ready\b/.test(content)) {
    return "Recreation plan ready.";
  }

  if (/\bplan is ready\b/.test(content)) {
    return "Plan ready.";
  }

  if (/\bscene detection\b/.test(content)) {
    return "Scene detection complete.";
  }

  if (/\b(?:analysis|understanding) (?:is|was) (?:complete|ready)\b|\bcomprehensive understanding\b/.test(content)) {
...
export function MessageBubble({
  ...
  const hasTaskTrace = !isUser && buildAssistantTimelineEntries(message.parts ?? []).length > 0;
  ...
  const processedTextParts = isUser
    ...
    : visibleAssistantTextParts.map(partText => {
        ...
        return Boolean(content) && (!hasTaskTrace || !isExecutionProgressNarration(content));
      });

  const finalReasoning = nativeReasoning || extractedReasoning;
  const visibleReasoning = hasTaskTrace ? null : finalReasoning;
  const historyReasoning = hasTaskTrace ? finalReasoning : null;
  const conciseExecutionAnalysis = hasTaskTrace
    ? getConciseExecutionAnalysis(visibleAssistantTextParts)
    : null;
      ...
      >
        {showTracePanel || hasAssistantErrors ? (
          <AssistantTracePanel
            ...
            isLoading={isLoading}
            analysisMessage={conciseExecutionAnalysis}
            historyReasoning={historyReasoning}
          />
        ) : null}
          ...
          <div className="w-full max-w-[48rem] rounded-[8px] border border-border/50 bg-muted/20 shadow-sm sm:pl-1">
            <details open={isReasoningOpen} onToggle={(e) => setIsReasoningOpen(e.currentTarget.open)} className="group">
              ...
              <div className="border-t border-border/40 px-4 py-3 text-xs leading-5 text-muted-foreground">
                <ChatMarkdown content={visibleReasoning} />
              </div>
            </details>
          </div>
        ) : null}
