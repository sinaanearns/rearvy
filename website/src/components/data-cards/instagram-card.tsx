...
import { normalizeInstagramPermalink } from "@/lib/integrations/instagram/links";
      ...
      >
        <div className="space-y-3">
          {data.posts.slice(0, 5).map((post, index) => {
            const permalink = normalizeInstagramPermalink(post.permalink);

            return (
              <div
                key={`${permalink || post.caption || "post"}:${index}`}
