...
export function Sidebar({
  ...
  function handleSelectAllVisible() {
    const visibleChatIds = (showAllChats ? recentChats : recentChats.slice(0, 5)).map((chat) => chat.id);

    if (visibleChatIds.length === 0) {
      return;
    }

    const allVisibleSelected = visibleChatIds.every((chatId) => selectedChatIds.includes(chatId));
    setSelectedChatIds((currentIds) => {
      if (allVisibleSelected) {
        return currentIds.filter((chatId) => !visibleChatIds.includes(chatId));
      }

      return Array.from(new Set([...currentIds, ...visibleChatIds]));
    });
  }
    ...
    try {
      ...
      if (deletedIds.length > 0) {
        ...
        if (isChatRoute) {
          ...
          const chatIndex = pathSegments.lastIndexOf("chat");
          if (chatIndex !== -1 && chatIndex + 1 < pathSegments.length) {
            const chatIdFromPath = pathSegments[chatIndex + 1];
            if (deletedIds.includes(chatIdFromPath)) {
              openFreshNewChat();
            }
          }
        }
  ...
  const allVisibleSelected =
    visibleChats.length > 0 && visibleChats.every((chat) => selectedChatIds.includes(chat.id));
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar">
        ...
        {/* Upgrade CTA — only on chat routes and expanded sidebar */}
        {!collapsed && isChatRoute ? (
          <div className={cn("py-1", collapsed ? "px-1.5" : "px-2")}>
            <SubscribeButton className="px-1" />
          </div>
        ) : null}
        ...
        {/* Your chats Section â€” hidden when collapsed */}
