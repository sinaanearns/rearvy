...
import { FeatureRequestButton } from "@/components/public/feature-request-button";
...
const publicNavLinks: NavLink[] = [
  { href: "/business/register", label: "Business Registration" },
...
export function RearvyPublicShell({
  ...
  const loginHref = authLinks?.login ?? "/login";
  const signupHref = authLinks?.signup ?? "/signup";
  const navigationLinks = navLinks ?? publicNavLinks;

  const showHero = Boolean(title || description || eyebrow || primaryCta || secondaryCta || stats?.length || sidePanel);

  return (
    <main
      className={["rearvy-home-grid min-h-screen w-full overflow-hidden pb-16 text-white selection:bg-purple-300 selection:text-black", className].filter(Boolean).join(" ")}
      style={style}
    >
