...
export function McpServersSection({ onServersChange }: McpServersSectionProps) {
    ...
    if (editingServer.type === "stdio" ? !editingServer.command?.trim() : !editingServer.url?.trim()) {
      toast.error(
        editingServer.type === "stdio"
          ? "A local MCP server requires a command."
          : "A web MCP server requires a URL."
      );
      return;
    }
  ...
  const remoteServerCount = servers.filter((server) => server.type !== "stdio").length;
            ...
            <div className="grid gap-2">
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="streamable_http">Streamable HTTP (Recommended)</SelectItem>
