...
test("forces browser task and routes to ChatGPT for image generation requests", () => {
  assert.equal(
    shouldForceBrowserTaskFirstStep("generate a PNG of a sunset over the ocean"),
    true
  );
  assert.equal(
    shouldForceBrowserTaskFirstStep("create an image of a cyberpunk city"),
    true
  );
  assert.equal(
    inferQuickStartUrl("generate a PNG of a sunset over the ocean"),
    "https://chatgpt.com"
  );

  const task = buildBrowserTaskInstruction({
    userText: "generate a PNG of a sunset over the ocean",
    startUrl: "https://chatgpt.com",
    targetLabel: "ChatGPT",
  });
  assert.match(task, /https:\/\/chatgpt\.com/);
  assert.match(task, /sunset over the ocean/);
  assert.match(task, /Downloads folder/);
});
