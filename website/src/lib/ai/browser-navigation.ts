...
export function inferQuickStartUrl(task: string, service?: string | null) {
  if (detectChatGptImageIntent(task)) {
...
export function buildBrowserTaskInstruction(params: {
  ...
  const chatGptImageIntent = detectChatGptImageIntent(trimmedUserText);
  if (chatGptImageIntent) {
    return buildChatGptImageTaskInstruction(chatGptImageIntent);
  }
  ...
  if (hasBrowserAuthIntent(trimmedUserText)) {
    ...
    return [
      ...
      `If a general landing page or auth prompt appears (such as "Log in or sign up"), click "Continue with Google" or the "Log In" button to proceed into the ${flowLabel} flow.`,
      ...
      "When the signup or login succeeds, immediately call saveMemory with a concise masked credential note in this exact format: 'Site: [site domain] | User: [the email used] | Note: password set by user'. Set memoryType to 'context', importance to 9, and tags to ['credential', 'login', site domain]. Do NOT include raw passwords, task prompts, or execution logs in memory.",
...
export function shouldForceBrowserTaskFirstStep(userText: string) {
  ...
  if (detectChatGptImageIntent(normalizedText)) {
    return true;
  }
