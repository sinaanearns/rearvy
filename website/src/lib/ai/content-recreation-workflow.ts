...
export const ContentRecreationShotSchema = z.object({
  ...
  timestamp: z.string().trim().max(40).default("0:00"),
  durationSeconds: z.number().min(0.5).max(60),
  visualDescription: z.string().trim().max(1200).default(""),
  inspirationNotes: z.string().trim().max(800).default(""),
  suggestedPrompt: z.string().trim().max(1600).default(""),
  onScreenText: z.string().trim().max(240).default(""),
  cameraAngle: z.string().trim().max(400).default("medium shot"),
  motionPlan: z.string().trim().max(800).default("static"),
  fusionPlan: z.string().trim().max(1000).default(""),
  moodAndColor: z.string().trim().max(500).default("neutral"),
...
export const ContentRecreationPlanSchema = z.object({
  ...
  targetNiche: z.string().trim().max(240).default("SaaS / Technology"),
  assumptions: z.array(z.string().trim().max(300)).min(1).max(8),
  confidenceScore: z.number().min(0).max(1).default(0.7),
  sourceUsePolicy: z.object({
    mode: z.enum(["reference_only", "owned_or_licensed_assets"]).default("reference_only"),
    notes: z.string().trim().max(700).default("Use for visual inspiration only."),
  }),
  overallStyle: z.string().trim().max(700).default(""),
  pinterestSearchQueries: z.array(z.string().trim().max(120)).min(1).max(8),
  shots: z.array(ContentRecreationShotSchema).min(1).max(24),
  davinciResolvePlan: z.object({
    timelineName: z.string().trim().max(120).default("Rearvy Timeline"),
    fps: z.number().int().min(24).max(60).default(30),
    aspectRatio: z.enum(["9:16", "16:9", "1:1", "4:5"]).default("9:16"),
    resolution: z.string().trim().max(30).default("1080x1920"),
    editSteps: z.array(z.string().trim().max(300)).min(1).max(24),
  }),
});
