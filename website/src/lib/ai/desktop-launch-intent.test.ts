...
test("ignores explicit MCP requests in desktop launch detectors", () => {
  const mcpPrompt = '"List my GitHub repositories." Using MCP server';

  assert.equal(isExplicitMcpRequest(mcpPrompt), true);
  assert.equal(detectDesktopLaunchIntent(mcpPrompt), null);
  assert.equal(hasClickyDesktopOperatorIntent(mcpPrompt), false);
  assert.equal(hasDirectDesktopWorkflowIntent(mcpPrompt), false);
});
