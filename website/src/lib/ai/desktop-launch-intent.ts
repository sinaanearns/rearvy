...
export type DesktopWorkflowAction =
  ...
  | {
      /** Import an FCPXML or EDL timeline into DaVinci Resolve and optionally render. */
      type: "importDaVinciTimeline";
      /** Absolute path to the .fcpxml or .edl file. */
      timelinePath: string;
      /** Display name used in logs (defaults to filename without extension). */
      timelineName?: string;
      /** If true, trigger Quick Export (Ctrl+Shift+E) immediately after import. */
      renderAfterImport?: boolean;
      /** Absolute path for the rendered output file. Defaults to Desktop/<name>_render.mp4. */
      renderOutputPath?: string;
      /** Extra milliseconds to wait after import before taking a screenshot (default 2500). */
      importSettleMs?: number;
      /** Maximum milliseconds to wait for render completion (default 120000). */
      renderWaitMs?: number;
    }
  | {
      /** Download a YouTube (or other yt-dlp-supported) video to disk. Requires yt-dlp on PATH. */
      type: "downloadYouTubeVideo";
      /** Full URL of the video to download. */
      url: string;
      /** yt-dlp output template path. Defaults to Desktop/%(title)s.%(ext)s. */
      outputPath?: string;
      /** yt-dlp format selector (default: bestvideo+bestaudio/best merged to mp4). */
      format?: string;
    };
...
export function isExplicitMcpRequest(userText: string | null | undefined): boolean {
  const text = normalizeText(userText);
  if (!text) return false;
  return /\busing\s+(?:an?\s+)?mcp\b|\bmcp\s+server\b|\bmcp\s+tool\b|\bmcp\b/i.test(text);
}

export function detectDesktopLaunchIntent(
  userText: string | null | undefined
): DesktopLaunchIntent | null {
  if (isWorkflowOutcomeMessage(userText) || isExplicitMcpRequest(userText)) {
    return null;
  }
  const text = normalizeText(userText);
  const match = text.match(DIRECT_LAUNCH_PATTERN);
