...
function cleanInlineMarkdown(value: string | null | undefined) {
  return String(value ?? "")
    .replace(/\*\*([^*]+)\*\*/g, "$1")
    .replace(/\*([^*]+)\*/g, "$1")
    .replace(/`([^`]+)`/g, "$1")
...
export function normalizeGeneratedDocumentMarkdown(
  ...
  if (!cleaned) {
    return `# ${fallbackTitle || "Generated Document"}\n\nDocument generation yielded no content.`;
  }
...
export function extractTitleFromMarkdown(markdown: string | null | undefined, fallback: string) {
  const safeMarkdown = String(markdown ?? "");
  const heading = safeMarkdown.match(/^#\s+(.+)$/m)?.[1];
...
export function createDocumentSummary(markdown: string | null | undefined) {
  const safeMarkdown = String(markdown ?? "");
  const firstParagraph = parseMarkdownBlocks(safeMarkdown).find(
...
export function analyzeGeneratedDocumentIntegrity(
  ...
  const placeholderMatches =
    safeMarkdown.match(/\[(?:NEEDS|PLACEHOLDER|VERIFY|INSERT|TODO):[^\]]+\]/gi) ??
    [];
...
function parseMarkdownBlocks(markdown: string | null | undefined): ParsedMarkdownBlock[] {
  const safeMarkdown = String(markdown ?? "");
  ...
  for (const rawLine of safeMarkdown.replace(/\r\n/g, "\n").split("\n")) {
...
function sanitizeFileBaseName(value: string | null | undefined) {
