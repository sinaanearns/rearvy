...
import type { WorkflowExecution } from "@/lib/firebase/schema";
import {
  OrchestratorPlan,
  OrchestratorStep,
...
export async function executeGoal(goal: string, ctx: ExecutionContext): Promise<ExecutionResult> {
  ...
  // Evolve multi-step business goals to use MultiMcpOrchestrator
  if (intent.requiresMultiStep || intent.category === "automation" || intent.category === "research" || intent.category === "media") {
    try {
      const { MultiMcpOrchestrator } = await import("@/lib/ai/mcp/orchestrator");
      const orchestration = await MultiMcpOrchestrator.orchestrateGoal({
        userId: ctx.userId,
        prompt: goal,
        isDesktopApp: ctx.isDesktopApp,
        allowedMcpServerIds: ctx.allowedMcpServerIds,
      });

      return {
        executionId: orchestration.executionId,
        workflowStatus: orchestration.status,
        understood: goal,
        plan: null,
        steps: orchestration.steps.map((s) => ({
          ...
          connectorName: s.connector_name ?? s.mcp_server_name,
        ...
        confidence: orchestration.confidenceScore,
        needsApproval: orchestration.needsApproval,
        assumptions: orchestration.assumptions,
        capabilitiesUsed: orchestration.capabilitiesUsed,
        connectorIdsUsed: orchestration.mcpServersUsed,
        intent,
      };
    } catch (orchError) {
      log.warn("MultiMcpOrchestrator fallback to standard planner:", orchError);
    }
  }
