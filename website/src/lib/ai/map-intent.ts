...
/** Matches requests that are clearly about programming rather than geography. */
const PROGRAMMING_CONTEXT_PATTERN =
  /\b(?:array|list|object|dict(?:ionary)?|function|iterate|iteration|filter|reduce|loop|forEach|callback|typescript|javascript|python|ruby|java|golang|rust|c\+\+|code|variable|method|iterable|collection|stream)\b/i;
...
export function detectMapGenerationIntent(
  ...
  // Exclude programming-context uses of "map" (e.g. "map over this array in TypeScript")
  if (PROGRAMMING_CONTEXT_PATTERN.test(text)) {
    return null;
  }
  ...
  // For any other map-related request, return a geocode-map intent
  return {
    kind: 'geocode-map',
    query: text,
    source: 'geocode-map-intent',
    assistantText: `Looking up location for "${text}"...`,
  };
}
