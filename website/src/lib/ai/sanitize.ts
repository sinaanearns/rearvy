...
const TEXT_APPROVAL_PROMPT_PATTERN =
  /\n*(?:#*\s*)?Approval Required\b[\s\S]*?\bWould you like me to proceed\b[\s\S]*$/i;
...
export function sanitizeAssistantText(text: unknown): string {
  ...
  sanitized = sanitized.replace(TEXT_APPROVAL_PROMPT_PATTERN, "").trim();
