...
async function searchDuckDuckGo(
  ...
  if (!response.ok) {
    log.warn("DuckDuckGo HTML search blocked", { status: response.status });
    return [];
  }
  ...
  try {
    liteResponse = await fetch(liteUrl, {
      headers: {
        "User-Agent": DEFAULT_USER_AGENT,
        Accept: "text/html,application/xhtml+xml",
      },
...
async function searchYahoo(
  ...
  if (!response.ok) {
    log.warn("Yahoo search blocked", { status: response.status });
    return [];
  }
...
async function searchBrave(
  ...
  try {
    ...
    if (!response.ok) {
      log.warn("Brave Search API failed", { status: response.status });
      return [];
    }
    ...
    return webResults.slice(0, limit).map((item: unknown, index: number) => ({
      title: isRecord(item) ? optionalString(item.title) : "",
      url: isRecord(item) ? optionalString(item.url) : "",
      snippet: isRecord(item) ? optionalString(item.description) : "",
      source: getHostname(isRecord(item) ? optionalString(item.url) : ""),
      rank: index + 1,
    }));
  } catch {
    return [];
...
async function searchSearXNG(
  ...
  // Use a reliable public SearXNG instance as a no-key fallback
  const instances = [
    "https://searx.be",
  ...
  for (const instance of instances) {
    try {
      const url = `${instance}/search?q=${encodeURIComponent(query)}&format=json&engines=google,bing,duckduckgo&categories=general`;
      const response = await fetch(url, {
        headers: {
          "User-Agent": DEFAULT_USER_AGENT,
          Accept: "application/json",
      ...
      const results: SearchResult[] = items
        ...
        .map((item: unknown, index: number) => ({
          title: isRecord(item) ? optionalString(item.title) : "",
          url: isRecord(item) ? optionalString(item.url) : "",
          snippet: isRecord(item) ? optionalString(item.content) : "",
          source: getHostname(isRecord(item) ? optionalString(item.url) : ""),
          rank: index + 1,
        }))
...
async function searchFirecrawl(
  ...
  try {
    // Use our v2 REST client for Firecrawl search
    const { firecrawlSearch: v2Search } = await import("@/lib/firecrawl/client");
    const response = await v2Search(query, { limit: Math.min(limit, 10) });
    ...
    log.warn("Firecrawl search failed", { err });
    return [];
  }
}
...
async function runSearchFallbacks(
  ...
  // 1. Firecrawl (best quality, requires FIRECRAWL_API_KEY)
  for (const candidateQuery of candidateQueries) {
    const fcResults = await searchFirecrawl(candidateQuery, limit);
    if (fcResults.length > 0) return fcResults;
  }

  // 2. Brave Search (free tier, requires BRAVE_SEARCH_API_KEY)
  for (const candidateQuery of candidateQueries) {
    const braveResults = await searchBrave(candidateQuery, limit);
  ...
  // 5. SearXNG public instances (aggregator, no key needed)
  for (const candidateQuery of candidateQueries) {
    const searxResults = await searchSearXNG(candidateQuery, limit);
...
export async function performWebSearch(
  ...
  try {
    ...
    if (results.length === 0) {
      ...
      method = results.length > 0 ? "fallbacks" : "none";
    }
...
async function fetchReadablePage(url: string): Promise<{
  title: string | null;
  content: string;
  method: "firecrawl" | "jina" | "direct";
}> {
  // 1. Firecrawl scrape â€” best quality clean markdown
  const app = getFirecrawl();
  if (app) {
    try {
      // FirecrawlClient v2: scrape() returns Document (FirecrawlDocument) directly
...
export async function performWebPageFetch(
  ...
  source?: string;
  fetchedAt?: string;
  fetchMethod?: "firecrawl" | "jina" | "direct";
  truncated?: boolean;
  content: string;
  errorCode?: string;
}> {
