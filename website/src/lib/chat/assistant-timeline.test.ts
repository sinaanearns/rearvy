...
test("buildAssistantTaskPanelState separates completed, current, and pending tasks", () => {
  const entries = buildAssistantTimelineEntries([
    {
      type: "tool-searchWeb",
      toolCallId: "complete",
    ...
    },
    {
      ...
      toolCallId: "current",
      state: "input-available",
      input: { path: "assets/scene3.svg" },
    },
    {
      type: "tool-generateDocument",
      toolCallId: "upcoming",
      state: "pending",
      input: { title: "Editing guide" },
    },
  ]);

  const state = buildAssistantTaskPanelState(entries);

  assert.deepEqual(state.completed.map((entry) => entry.key), ["complete"]);
  assert.equal(state.current?.key, "current");
  assert.deepEqual(state.upcoming.map((entry) => entry.key), ["upcoming"]);
...
test("formatDurationMs formats sub-second and multi-second durations cleanly", () => {
  ...
  assert.equal(formatDurationMs(15200), "15.2s");
