...
export const DEFAULT_LOCAL_CLICKY_API_PORT = 4000;
...
export async function resolveLocalClickyApiPort(preferredPort?: number | null) {
  ...
  return DEFAULT_LOCAL_CLICKY_API_PORT;
}

export function buildLocalClickyApiBaseUrl(port: number) {
  return `http://127.0.0.1:${port}`;
}

export async function probeLocalClickyVoiceService(options: {
  localApiPort?: number | null;
  timeoutMs?: number;
} = {}) {
  ...
  const port = await resolveLocalClickyApiPort(options.localApiPort);
  const baseUrl = buildLocalClickyApiBaseUrl(port);
...
export async function transcribeWithLocalClicky(
  blob: Blob,
  ...
  const localApiPort = await resolveLocalClickyApiPort(options.localApiPort ?? metadata.localApiPort);
  const endpoint = `${buildLocalClickyApiBaseUrl(localApiPort)}/api/internal/clicky/transcribe`;
  ...
  try {
    ...
    throw new LocalVoiceTranscriptionError(
      "Clicky voice service is not running.",
