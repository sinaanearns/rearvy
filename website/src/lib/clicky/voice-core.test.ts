...
function dictionaryEntry(partial: Partial<ClickyVoiceDictionaryEntry>): ClickyVoiceDictionaryEntry {
  return {
    id: partial.id || "dict",
    userId: "user",
...
function snippet(partial: Partial<ClickyVoiceSnippet>): ClickyVoiceSnippet {
  return {
...
test("dictionary keyterms are clamped for AssemblyAI prompts", () => {
  const terms = getDictionaryKeyterms([
    dictionaryEntry({ replacement: "Rearvy", keyterms: ["Clicky", "too many words in this phrase should be ignored now"] }),
  ]);

  assert.deepEqual(terms, ["Rearvy", "Clicky"]);
});
