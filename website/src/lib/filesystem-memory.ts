...
import { condenseMemoryText, isTaskOrLogText } from "@/lib/memory-store";
...
/**
 * Resolve the memory root outside the repository.
 *
 * Priority:
 *   1. MEMORY_ROOT env var (absolute path wins immediately)
 *   2. %LOCALAPPDATA%\Rearvy\Memory  (Windows)
 *   3. ~/.rearvy/Memory              (macOS / Linux)
 *
 * The repo directory is NEVER used as a fallback so that personal
 * data never ends up committed to GitHub.
 */
function resolveMemoryRoot(): string {
  if (process.env.MEMORY_ROOT) {
    return path.resolve(process.env.MEMORY_ROOT);
  }
  if (process.platform === "win32") {
    const localAppData = process.env.LOCALAPPDATA || path.join(os.homedir(), "AppData", "Local");
    return path.join(localAppData, "Rearvy", "Memory");
  }
  return path.join(os.homedir(), ".rearvy", "Memory");
}
...
function entryKey(content: string) {
  ...
  const siteMatch = clean.match(/(?:site:\s*|domain:\s*|for\s+|at\s+|on\s+)([a-z0-9.-]+\.[a-z]{2,}|[a-z0-9_-]{3,})/i);
  if (siteMatch && siteMatch[1]) {
    return `${siteMatch[1].toLowerCase()} Credential`;
  }
  const match = clean.match(/^(?:my|our|the)?\s*([a-z0-9][a-z0-9 _-]{1,36})\s+(?:is|are|changed|updated|number|phone|email|password|preference|goal)/i);
  const derived = (match?.[1] || clean.slice(0, 32)).trim();
  return safe(derived).replace(/_/g, " ");
}
...
export async function saveFileMemory(input: FileMemoryInput): Promise<FileMemoryResult> {
  if (isTaskOrLogText(input.content)) {
    return { category: "Unknown", filePath: "", created: false, updated: false };
  }
  const condensed = condenseMemoryText(input.content);
  const content = (condensed || input.content).replace(/\s+/g, " ").trim();
