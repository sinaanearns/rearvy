...
export const COLLECTIONS = {
  ...
  // Admin — Business registrations from public form
  BUSINESS_REGISTRATIONS: "business_registrations",

  // Admin — Feature requests from users
  FEATURE_REQUESTS: "feature_requests",

  // Rearvy Organization & MCP Platform
  ORGANIZATIONS: "organizations",
  ORGANIZATION_MEMBERS: "organization_members",
  MCP_PERMISSIONS: "mcp_permissions",
  BUSINESS_KNOWLEDGE_NODES: "business_knowledge_nodes",
  BUSINESS_KNOWLEDGE_EDGES: "business_knowledge_edges",
...
export interface Organization {
  ...
  brand_memory?: {
    ...
    tone?: string;
    target_kpis?: Record<string, string | number>;
  };
  created_at: Date | string;
  updated_at: Date | string;
}
...
export interface McpPermission {
  ...
  scope: string; // e.g. "send_emails", "read_orders", "update_inventory"
  granted: boolean;
...
export interface BusinessKnowledgeNode {
  id: string;
  org_id?: string;
  user_id: string;
  type: "Customer" | "Order" | "Revenue" | "Product" | "Employee" | "Inventory" | "Marketing" | "Finance" | "Support" | "Document";
  label: string;
...
export interface BusinessKnowledgeEdge {
  id: string;
  source_id: string;
  target_id: string;
  relationship: string; // e.g. "PURCHASED", "MANAGES", "PROMOTES", "CONTAINS"
  metadata?: Record<string, unknown>;
  created_at: Date | string;
}
...
export interface MarketplaceApp {
  ...
  title: string;
  description: string;
  category: "CRM" | "Marketing" | "Sales" | "Finance" | "HR" | "Support" | "Analytics" | "Productivity" | "Development" | "Design" | "AI";
  icon_name: string;
  capabilities: string[];
  mcp_config_template: {
    name: string;
...
export interface ConnectorDefinitionRecord {
  ...
  lifecycle_status: ConnectorLifecycleStatus;
  contract_validation: {
    passed: boolean;
    validated_at: string;
    errors: string[];
...
export interface WorkflowExecutionStep {
  ...
  connector_transport?: "mcp" | "oauth_api" | "api_key" | "webhook" | "local_bridge" | "browser_extension";
  operation_id?: string;
  operation_name?: string;
  operation_input_schema?: Record<string, unknown>;
  risk?: "read" | "write" | "publish" | "destructive";
  approval_required?: boolean;
  depends_on?: string[];
  attempt_count?: number;
  ...
  approved_at?: string;
  approved_by?: string;
  /** @deprecated Use connector_id. Retained for existing workflow records. */
  mcp_server_id?: string;
  /** @deprecated Use connector_name. Retained for existing workflow records. */
