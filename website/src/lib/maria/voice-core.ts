export type MariaVoiceScope = "personal" | "team";
export type MariaVoiceMode = "dictation" | "command";
export type MariaVoiceRetentionMode = "off" | "metadata" | "transcripts";
export type MariaVoiceLanguageMode = "auto" | "english" | "multilingual";
export type MariaVoiceAppCategory =
  | "email"
  | "chat"
  | "docs"
  | "code"
...
export type MariaVoiceProfile = {
  ...
  languageMode: MariaVoiceLanguageMode;
  retentionMode: MariaVoiceRetentionMode;
  usageAnalyticsVisible: boolean;
  styleDefaults: Partial<Record<MariaVoiceAppCategory, string>>;
  createdAt?: string;
  updatedAt?: string;
};

export type MariaVoiceDictionaryEntry = {
  id: string;
  userId: string;
  teamId?: string | null;
  scope: MariaVoiceScope;
  spoken: string;
  replacement: string;
  keyterms: string[];
  priority: number;
...
export type MariaVoiceStyle = {
  id: string;
  userId: string;
  teamId?: string | null;
  scope: MariaVoiceScope;
  name: string;
  category: MariaVoiceAppCategory;
  instructions: string;
  enabled: boolean;
  createdAt?: string;
  updatedAt?: string;
...
export type MariaVoiceTeam = {
  id: string;
  ownerId: string;
  name: string;
  settings: MariaVoiceTeamSettings;
  createdAt?: string;
  updatedAt?: string;
};

export type MariaVoiceTeamMemberRole = "owner" | "admin" | "member";

export type MariaVoiceTeamMember = {
  id: string;
  teamId: string;
  userId: string;
  email: string | null;
  role: MariaVoiceTeamMemberRole;
  createdAt?: string;
  updatedAt?: string;
};

export type MariaVoiceTeamSettings = {
  contextAwarenessEnabled: boolean;
  retentionMode: MariaVoiceRetentionMode;
  usageAnalyticsVisible: boolean;
};

export type MariaVoiceActiveContext = {
  appName?: string | null;
  title?: string | null;
  url?: string | null;
  category?: MariaVoiceAppCategory | null;
  workspacePath?: string | null;
  workspaceFiles?: string[];
};

export type MariaVoiceProcessingInput = {
  transcript: string;
  mode?: MariaVoiceMode;
  command?: string | null;
  selectedText?: string | null;
  profile?: Partial<MariaVoiceProfile> | null;
...
export type MariaVoiceProcessingOutput = {
  text: string;
  mode: MariaVoiceMode;
  pressEnter: boolean;
...
export const DEFAULT_MARIA_VOICE_PROFILE: MariaVoiceProfile = {
  userId: "",
...
function addTerminalPunctuation(value: string, category: MariaVoiceAppCategory) {
...
export function normalizeVoiceProfile(userId: string, value?: Partial<MariaVoiceProfile> | null): MariaVoiceProfile {
  const retentionMode =
    value?.retentionMode === "metadata" || value?.retentionMode === "transcripts"
      ? value.retentionMode
      : DEFAULT_MARIA_VOICE_PROFILE.retentionMode;
  const languageMode =
    value?.languageMode === "english" || value?.languageMode === "multilingual"
      ? value.languageMode
      : DEFAULT_MARIA_VOICE_PROFILE.languageMode;

  return {
    ...DEFAULT_MARIA_VOICE_PROFILE,
    ...value,
    userId,
    shortcut: readString(value?.shortcut, DEFAULT_MARIA_VOICE_PROFILE.shortcut),
    commandShortcut: readString(value?.commandShortcut, DEFAULT_MARIA_VOICE_PROFILE.commandShortcut),
    ...
    languageMode,
    styleDefaults: {
      ...DEFAULT_MARIA_VOICE_PROFILE.styleDefaults,
      ...(value?.styleDefaults || {}),
    },
  };
}

export function inferVoiceCategory(activeContext?: MariaVoiceActiveContext | null): MariaVoiceAppCategory {
...
function sortByVoicePriority<T extends { scope: MariaVoiceScope; priority: number; id: string }>(items: T[]) {
...
export function applyDictionary(value: string, entries: MariaVoiceDictionaryEntry[] = []) {
  let text = value;
...
export function applySnippets(value: string, snippets: MariaVoiceSnippet[] = []) {
  let text = value;
...
function finalizeWritingText(value: string, category: MariaVoiceAppCategory) {
...
export function getDictionaryKeyterms(entries: MariaVoiceDictionaryEntry[] = []) {
...
export function processVoiceTranscript(input: MariaVoiceProcessingInput): MariaVoiceProcessingOutput {
  const mode: MariaVoiceMode = input.mode === "command" ? "command" : "dictation";
