...
import {
  DEFAULT_MARIA_VOICE_PROFILE,
  getDictionaryKeyterms,
  normalizeVoiceProfile,
  type MariaVoiceDictionaryEntry,
  type MariaVoiceProfile,
  type MariaVoiceScope,
  type MariaVoiceSnippet,
  type MariaVoiceStyle,
  type MariaVoiceTeam,
  type MariaVoiceTeamMember,
  type MariaVoiceTeamMemberRole,
  type MariaVoiceTeamSettings,
} from "@/lib/maria/voice-core";

export type MariaVoiceContext = {
  profile: MariaVoiceProfile;
  dictionary: MariaVoiceDictionaryEntry[];
  snippets: MariaVoiceSnippet[];
  styles: MariaVoiceStyle[];
  teams: MariaVoiceTeam[];
  memberships: MariaVoiceTeamMember[];
  keyterms: string[];
};

export type MariaVoiceUsageSummary = {
  totalEvents: number;
  totalWords: number;
...
const DEFAULT_TEAM_SETTINGS: MariaVoiceTeamSettings = {
...
function normalizeScope(value: unknown): MariaVoiceScope {
  return value === "team" ? "team" : "personal";
}

function normalizeRole(value: unknown): MariaVoiceTeamMemberRole {
...
function mapDictionaryDoc(id: string, data: Record<string, unknown>): MariaVoiceDictionaryEntry {
  return {
    id,
...
function mapSnippetDoc(id: string, data: Record<string, unknown>): MariaVoiceSnippet {
  return {
    id,
    userId: readString(data.user_id || data.userId),
    teamId: readString(data.team_id || data.teamId) || null,
...
function mapStyleDoc(id: string, data: Record<string, unknown>): MariaVoiceStyle {
...
function mapTeamDoc(id: string, data: Record<string, unknown>): MariaVoiceTeam {
  return {
    id,
    ownerId: readString(data.owner_id || data.ownerId),
    name: readString(data.name, "Maria Team", 160),
...
function mapMemberDoc(id: string, data: Record<string, unknown>): MariaVoiceTeamMember {
  return {
    id,
...
export async function getVoiceProfile(db: Firestore, userId: string) {
  const snap = await db.collection(COLLECTIONS.MARIA_VOICE_PROFILES).doc(userId).get();
  const data = snap.data() || {};
  return normalizeVoiceProfile(userId, {
    shortcut: readString(data.shortcut, DEFAULT_MARIA_VOICE_PROFILE.shortcut),
    commandShortcut: readString(data.command_shortcut || data.commandShortcut, DEFAULT_MARIA_VOICE_PROFILE.commandShortcut),
    ...
    styleDefaults:
      data.style_defaults && typeof data.style_defaults === "object"
        ? (data.style_defaults as MariaVoiceProfile["styleDefaults"])
        : data.styleDefaults && typeof data.styleDefaults === "object"
          ? (data.styleDefaults as MariaVoiceProfile["styleDefaults"])
          : {},
    createdAt: readString(data.created_at || data.createdAt),
    updatedAt: readString(data.updated_at || data.updatedAt),
  });
...
export async function listVoiceMemberships(db: Firestore, userId: string) {
  const snapshot = await db
    .collection(COLLECTIONS.MARIA_VOICE_TEAM_MEMBERS)
...
export async function listVoiceTeams(db: Firestore, userId: string) {
  const [ownedSnapshot, memberships] = await Promise.all([
    db.collection(COLLECTIONS.MARIA_VOICE_TEAMS).where("owner_id", "==", userId).get(),
    listVoiceMemberships(db, userId),
  ]);

  const teams = new Map<string, MariaVoiceTeam>();
  ...
  await Promise.all(
    memberships.map(async (membership) => {
      ...
      const snap = await db.collection(COLLECTIONS.MARIA_VOICE_TEAMS).doc(membership.teamId).get();
      if (snap.exists) {
...
export async function getVoiceTeamAccess(db: Firestore, teamId: string, userId: string) {
  const teamSnap = await db.collection(COLLECTIONS.MARIA_VOICE_TEAMS).doc(teamId).get();
  if (!teamSnap.exists) {
    return null;
  }
  ...
  if (team.ownerId === userId) {
    return { team, role: "owner" as MariaVoiceTeamMemberRole, canManage: true };
  }
...
export async function createVoiceTeam(db: Firestore, userId: string, name: string, email: string | null) {
  const ref = db.collection(COLLECTIONS.MARIA_VOICE_TEAMS).doc();
  const memberRef = db.collection(COLLECTIONS.MARIA_VOICE_TEAM_MEMBERS).doc();
  const now = nowIso();
  const team = {
    owner_id: userId,
    name: readString(name, "Maria Team", 160),
...
export async function getVoiceContext(db: Firestore, userId: string): Promise<MariaVoiceContext> {
  const [profile, memberships, personalDictionary, personalSnippets, personalStyles] = await Promise.all([
    getVoiceProfile(db, userId),
    listVoiceMemberships(db, userId),
    db.collection(COLLECTIONS.MARIA_VOICE_DICTIONARY).where("user_id", "==", userId).where("scope", "==", "personal").get(),
    db.collection(COLLECTIONS.MARIA_VOICE_SNIPPETS).where("user_id", "==", userId).where("scope", "==", "personal").get(),
    db.collection(COLLECTIONS.MARIA_VOICE_STYLES).where("user_id", "==", userId).where("scope", "==", "personal").get(),
  ]);
  ...
  const [teams, teamDictionary, teamSnippets, teamStyles] = await Promise.all([
    ...
    Promise.all(
      teamIds.map((teamId) =>
        db.collection(COLLECTIONS.MARIA_VOICE_DICTIONARY).where("team_id", "==", teamId).where("scope", "==", "team").get()
      )
    ),
    Promise.all(
      teamIds.map((teamId) =>
        db.collection(COLLECTIONS.MARIA_VOICE_SNIPPETS).where("team_id", "==", teamId).where("scope", "==", "team").get()
      )
    ),
    Promise.all(
      teamIds.map((teamId) =>
        db.collection(COLLECTIONS.MARIA_VOICE_STYLES).where("team_id", "==", teamId).where("scope", "==", "team").get()
      )
    ),
  ]);
...
export async function createDictionaryEntry(
  ...
  const ref = db.collection(COLLECTIONS.MARIA_VOICE_DICTIONARY).doc();
  const entry = {
    user_id: userId,
...
export async function createSnippet(
  ...
  const now = nowIso();
  const scope = normalizeScope(input.scope);
  const ref = db.collection(COLLECTIONS.MARIA_VOICE_SNIPPETS).doc();
  const snippet = {
    user_id: userId,
    team_id: scope === "team" ? readString(input.teamId || input.team_id, "", 200) || null : null,
    scope,
...
export async function createStyle(
  ...
  const ref = db.collection(COLLECTIONS.MARIA_VOICE_STYLES).doc();
  const style = {
    user_id: userId,
    team_id: scope === "team" ? readString(input.teamId || input.team_id, "", 200) || null : null,
    scope,
...
export async function updateVoiceResource(
  ...
  if (collection === COLLECTIONS.MARIA_VOICE_DICTIONARY) return mapDictionaryDoc(id, next);
  if (collection === COLLECTIONS.MARIA_VOICE_SNIPPETS) return mapSnippetDoc(id, next);
  if (collection === COLLECTIONS.MARIA_VOICE_STYLES) return mapStyleDoc(id, next);
  return { id, ...next };
}
...
export async function recordVoiceUsage(
  ...
  const ref = db.collection(COLLECTIONS.MARIA_VOICE_USAGE_EVENTS).doc();
  const event = {
    user_id: userId,
...
export async function getVoiceUsageSummary(db: Firestore, userId: string): Promise<MariaVoiceUsageSummary> {
  const snapshot = await db
