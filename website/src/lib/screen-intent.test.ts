...
test("extracts target app from background screen intent requests", () => {
  const { extractTargetAppFromScreenIntent, detectScreenIntent } = require("./screen-intent.ts");
  
  assert.equal(extractTargetAppFromScreenIntent("take a screenshot of DaVinci Resolve"), "davinci resolve");
  assert.equal(extractTargetAppFromScreenIntent("capture screenshot of VS Code in the background"), "vs code");
  assert.equal(extractTargetAppFromScreenIntent("inspect DaVinci Resolve in the background"), "davinci resolve");
  
  const result = detectScreenIntent("take a screenshot of DaVinci Resolve");
  assert.equal(result.isScreenRead, true);
  assert.equal(result.targetApp, "davinci resolve");
});
